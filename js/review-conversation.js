...
__fluffyModules["review-conversation.js"] = (() => {
    ...
    /** 输入：模型返回。输出：验证后的短句和转写。功能：拒绝错误或空响应，禁止模型输出进入HTML。 */
    function parse(result, audio = false) {
        const choice = result?.choices?.[0];
        if (!choice || choice.finish_reason && choice.finish_reason !== "stop") throw Error("小猫的话还没说完整，请再试一次。");
        let body;
        try { body = JSON.parse(String(choice.message?.content || "").replace(/^```(?:json)?\s*|\s*```$/g, "")); } catch { throw Error("回复格式没有整理好，请再试一次。"); }
        if (!Array.isArray(body.replies)) throw Error("没有收到完整回复，请再试一次。");
        const transcript = typeof body.transcript === "string" ? body.transcript.trim().slice(0, 4000) : "";
        if (audio && !transcript) throw Error("这次没有听清，再说一次吧。");
        ...
        if (!replies.length) throw Error("没有收到小猫的回复，请再试一次。");
        return { transcript, replies };
    }
    /** 输入：客户端、只读回顾、对话、文本/音频、语言与取消信号。输出：经过验证的回应。功能：真正调用模型，且只传当前板块资料。 */
    async function request({ api, bailian, view, history = [], text = "", audio = null, lang = "zh", opening = false, signal }) {
        ...
        const messages = [{ role: "system", content: prompt(lang, opening, Boolean(audio)) }, { role: "user", content: JSON.stringify({ context: Data.context(view) }) }, ...prior];
        const instruction = opening ? (lang === "en" ? "Please greet me about this record." : "看看这份记录，和我聊一句吧。") : String(text).trim().slice(0, 4000);
        if (audio && (!/^data:audio\/wav;base64,[A-Za-z0-9+/=]+$/.test(audio.data || "") || audio.data.length > 5e6)) throw Error("录音格式或长度不合适，请重试。");
        ...
        return parse(response, Boolean(audio));
    }
    /** 输入：句子、实际测量函数、可用宽度。输出：每页一至两行的完整短句片段。功能：平衡两行，按词/字分页，不截掉回复。 */
    function pages(text, measure, width) {
        ...
        while (units.length) {
            ...
            for (let n = 1; n <= units.length; n++) {
                ...
                if (measure(chunk) <= width) { take = n; good = [chunk]; continue; }
                let split = null, cost = Infinity;
                for (let j = 1; j < n; j++) {
                    const a = units.slice(0, j).join("").trim(), b = units.slice(j, n).join("").trim(), wa = measure(a), wb = measure(b);
                    if (!a || !b || wa > width || wb > width || /^[，。！？、；：,.!?;:]/.test(b)) continue;
                    const penalty = Math.abs(wa - wb) + (/[，；。！？,.!?;]$/.test(a) ? -8 : 0);
                    if (penalty < cost) { cost = penalty; split = [a, b]; }
                }
                if (!split) break;
                take = n; good = split;
                if (/[。！？.!?]$/.test(chunk) && measure(chunk) > width) break;
            }
            if (!take) { take = 1; good = [units[0]]; }
            output.push(good); units.splice(0, take);
        }
        return output;
    }
