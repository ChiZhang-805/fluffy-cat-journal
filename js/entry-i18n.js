...
__fluffyModules["entry-i18n.js"] = (() => {
    ...
    let current = "zh";
    try { current = localStorage.getItem(KEY) === '"en"' ? "en" : "zh"; } catch { /* 无存储权限时本页仍可切换。 */ }
    const TEXT = Object.freeze({
        ...
        "确认并继续": "Confirm & continue", "开始专注": "Start focusing", "补记专注": "Log past focus", "修改专注": "Edit focus session", "保存补记": "Save this session", "保存修改": "Save changes", "完成": "Done", "继续": "Continue", "修改记录": "Edit entry",
    ...
    function setLanguage(value) {
        ...
        try { localStorage.setItem(KEY, JSON.stringify(current)); } catch { /* 存储不可用也不阻止本页切换。 */ }
