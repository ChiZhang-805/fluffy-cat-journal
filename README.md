# Fluffy Cat · v14 完整手写笔记

本版基于已发布的 v13。只改共用笔记布局、写完后的有限阅读、完整字段投影及相关测试；
不改变小猫原画、AI服务路由、照片自动分析、日期补记、语言设置、记录存储和回顾统计。

## v14 新增与修复

每个字段根据实际笔画行数占据自然高度。中文与英文均以25设计像素生成手写内容；
长内容换行延长纸面，不靠缩小字号塞满卡片。上一字段末行横线到下一标题顶部固定留30设计像素。

书写：每一视觉行在笔尖下方停稳，换行/换字段时抬笔送纸；长字段保留当前项标题。
末笔之后继续向上归位到下一圈的第一项，再切换到只有一份内容的原生滚动层。
切换时墨迹仍由同一个Canvas和同一份笔画绘制，没有两张图的淡入淡出。

完成：从第一项开始，第一项上方没有上一圈的尾字或横线；手指、滚轮、触控板和键盘都能有限阅读。
到顶就停，到底就停，小猫呼吸时不会把scrollTop重置。卡片、猫和继续按钮不随笔记滚动。
三项单行内容不足一屏时，没有额外空白滚动。

删除了书写/完成页上的“修改记录”链接；回顾菜单与历史里的编辑功能仍保留。
饮食完整展示食物、份量、热量、蛋白质、碳水、脂肪、备注；面部与睡眠也不再把字段缩成三项摘要。

## 使用与更新

以静态站点部署运行 `index.html`。双击外层 `Update-GitHub-Pages.cmd` 更新现有仓库；
请勿使用旧的 `Publish-GitHub-Pages.cmd` 新建仓库。
开发检查：`npm test`；`npm run test:browser`；离线预览：`npm run build:standalone`。

详细实现和验证范围见 `docs/v14-notebook.md`。

---

以下保留上一版本的AI、隐私、部署和配置说明：
