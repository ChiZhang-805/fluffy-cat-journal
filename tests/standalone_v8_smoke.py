...
SOURCE=Path(sys.argv[1]) if len(sys.argv)>1 else ROOT.parent/'Fluffy-Cat-v8.html'
OUT=Path(os.environ.get('FLUFFY_RESULTS',ROOT/'tests/results/v8-standalone'));OUT.mkdir(parents=True,exist_ok=True)
RESULTS=[]
...
def run():
    ...
    with sync_playwright()as p:
        ...
        for id in ['sport','food','mood','sleep','face','focus']:
            page.evaluate('(id)=>FluffyDebug.openEntry(id)',id);page.locator('#entry-more').click();check(id+'单文件有三项菜单',page.locator('#entry-menu button').count()==3);page.keyboard.press('Escape')
        page.evaluate("FluffyDebug.openEntry('sport',{activity:'跑步',durationMinutes:12,notes:'沿河慢跑'})")
        page.locator('#entry-more').click();page.locator('[data-action=date]').click();page.locator('[data-date]:not(:disabled)').nth(6).click();date=page.evaluate('FluffyDebug.entryMenu.candidate');page.locator('.date-save').click();page.locator('#confirm-entry').click()
        ...
        check('单文件无脚本错误',not errors);page.close();b.close()
