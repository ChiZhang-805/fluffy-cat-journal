...
test('v12回顾拒绝原始音频直传，必须独立听写再交DeepSeek',async()=>{let called=false;const bailian={configured:true,provider:'bailian',request:async()=>{called=true;}};await assert.rejects(T.request({api:{configured:true},bailian,view:D.build('mood','2026-09-22',[]),audio:{data:'data:audio/wav;base64,AAAA'},text:''}));assert.equal(called,false);});
