...
def test_document(extra_js=''):
    ...
    for link in soup.find_all('link',rel='stylesheet'):
        style=soup.new_tag('style');style.string=(ROOT/link['href'].split('?')[0]).read_text();link.replace_with(style)
    ...
    head=soup.new_tag('script');head.string='window.FLUFFY_TEST=true;window.CAT_ASSETS='+json.dumps(assets)+';\n'+extra_js
