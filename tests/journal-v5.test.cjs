...
test('音频真实input_audio加stream文字模式',async()=>{let payload;const api={provider:'bailian',model:'vision',audioModel:'omni',routes:{chat:'/chat'},request:async(p,b)=>{payload=b;return response({fields:{mood:'开心'},transcript:'我今天很开心',emotion:{basis:'explicit'}});}};const r=await AI.extractAudio(api,'mood',{data:'data:audio/wav;base64,UklGRg==',format:'wav'},'',undefined,anchor);assert.equal(payload.model,'omni');assert.equal(payload.stream,true);assert.equal(payload.enable_thinking,false);assert.equal(payload.modalities[0],'text');assert.equal(payload.messages[1].content[0].input_audio.data,'data:;base64,UklGRg==');assert.equal(r.fields.mood,'开心');});
