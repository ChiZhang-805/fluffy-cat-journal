...
async def run(target):
    ...
    check('产物带有v7版本标识', 'fluffy-recap-20260922-v7' in html)
    ...
    async with async_playwright() as p:
        ...
        await page.locator('#primary').click()
        check('交付HTML庆祝完成进入运动回顾', await page.evaluate('FluffyDebug.animation.scene==="review"&&FluffyDebug.review.view.id==="sport"'))
        await page.wait_for_timeout(1000)
        check('交付HTML回顾读取真实输入的35分钟', await page.locator('.review-fact-value').inner_text()=='35 分钟')
        await page.locator('#review-more').click()
        check('交付HTML菜单四字短语', all(len(v)==4 for v in await page.locator('#review-menu button span').all_inner_texts()))
        await page.keyboard.press('Escape')
        await page.locator('#review-home').click()
        check('交付HTML回顾房屋按钮返回原首页', await page.evaluate('FluffyDebug.animation.scene==="home"'))
...
if __name__ == '__main__':
    try:
        ...
        out = Path(__file__).resolve().parent / 'results' / 'standalone-v7-results.json'
